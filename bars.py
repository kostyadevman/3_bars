...
def get_smallest_bar(json_content):
    smallest_bar = min(json_content['features'],
                       key=lambda bar: bar['properties']['Attributes']['SeatsCount'])
    return smallest_bar


def get_closest_bar(json_content, longitude, latitude):
    closest_bar = min(json_content['features'],
                      key=lambda bar: distance(bar['geometry']['coordinates'], (longitude, latitude)).km)
    return closest_bar
...
if __name__ == '__main__':
    ...
    if json_content:
        ...
        seats_count_biggest_bar = biggest_bar['properties']['Attributes']['SeatsCount']

        name_smallest_bar = smallest_bar['properties']['Attributes']['Name']
        seats_count_smallest_bar = smallest_bar['properties']['Attributes']['SeatsCount']

        name_closest_bar = closest_bar['properties']['Attributes']['Name']
        distance_closet_bar = distance(closest_bar['geometry']['coordinates'], (my_longtude, my_latitude)).meters

        print(u'The biggest bar: {} with {} seats'.format(name_biggest_bar, seats_count_biggest_bar))
        print(u'The smallest bar: {} with {} seats'.format(name_smallest_bar, seats_count_smallest_bar))
        print(u'The closest bar: {} distance: {} meters'.format(name_closest_bar, round(distance_closet_bar,2)))
